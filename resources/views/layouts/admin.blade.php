...
<head>
    ...
    <title>@yield('title', 'COMMAND_CENTER') // MATAJALAN_OS</title>
...
<body class="bg-slate-950 text-slate-300 font-sans selection:bg-cyan-500/30 selection:text-cyan-200 h-screen flex overflow-hidden">
    ...
    <aside class="w-64 bg-slate-900 border-r border-slate-800 flex flex-col shrink-0">
        ...
        <nav class="flex-1 overflow-y-auto p-4 space-y-1">
            ...
            <a href="{{ route('admin.vehicle-edits.index') }}" class="flex items-center gap-3 px-4 py-3 text-xs font-mono rounded hover:bg-slate-800 {{ request()->routeIs('admin.vehicle-edits.*') ? 'bg-slate-800 text-cyan-400 border-l-2 border-cyan-500' : 'text-slate-400 border-l-2 border-transparent' }}">
                <i data-lucide="file-edit" class="w-4 h-4"></i>
                VEHICLE_EDITS
                @if(\App\Models\VehicleEdit::where('status', 'pending')->count() > 0)
                    <span class="ml-auto bg-blue-500 text-white text-[10px] px-1.5 rounded-full font-bold">!</span>
                @endif
            </a>
